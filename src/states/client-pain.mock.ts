import { PainProps } from "@/@types";
import { atom } from "jotai";

export const clientPainInformation = atom<PainProps[]>([
  {
    id: 1,
    title: "Visibilidade",
    description:
      "A falta de um site na internet significa que a sua empresa ou marca não está visível online. Isso pode fazer com que você perca oportunidades de negócios para concorrentes que possuem um site e estão visíveis na internet.",
  },
  {
    id: 2,
    title: "Alcance",
    description:
      "Sem um site na internet, a sua empresa ou marca está limitada ao seu alcance geográfico e ao seu público local. Isso pode dificultar a atração de novos clientes e expandir seus negócios.",
  },
  {
    id: 3,
    title: "Credibilidade",
    description:
      "A ausência de um site pode levar os clientes a questionarem a credibilidade da sua empresa ou marca. Isso pode prejudicar a confiança e a reputação do seu negócio.",
  },
  {
    id: 4,
    title: "Comunicação",
    description:
      "Sem um site, a sua empresa ou marca pode ter dificuldade em se comunicar com seus clientes de forma eficiente e eficaz. Isso pode resultar em atrasos na resolução de problemas e na prestação de serviços.",
  },
  {
    id: 5,
    title: "Marketing",
    description:
      "Sem um site, a sua empresa ou marca pode perder oportunidades de marketing valiosas, como publicidade online, marketing por e-mail e promoções especiais. Isso pode afetar negativamente o crescimento e o sucesso do seu negócio.",
  },
]);
